# Value representation for native codegen: Julia type → Cranelift type mapping.
# Originally shared with the (now-removed) WasmCodegen; the NumType enum was
# WasmTools' NumType — defined locally now.
#
...
# exact same representation native Julia uses — both inside the compiled code
# and on the wire. This makes bitcast/zext/trunc on Char identity operations
# and keeps Base's codepoint decoding (e.g. `UInt32(::Char)`) bit-faithful. Do
# NOT store the codepoint anywhere: mixing the two conventions silently corrupts
# every Char that crosses the boundary.

@enum NumType::UInt8 I64=0 I32=1 F64=2 F32=3

struct ScalarRepr
    vt::NumType      # storage type
...
`Ptr` is deliberately excluded (host pointers must not flow into compiled code).
"""
...
"""Convert a value arriving as i32/i64/f32/f64 back to Julia type `T`."""
...
"""Convert a Julia scalar to what the boundary expects."""
